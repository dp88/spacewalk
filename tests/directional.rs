...
fn a_river_runs_one_way() {
    ...
    let top = g.at(Sq::new(2, 0));
    let bottom = g.at(Sq::new(2, 4));
...
fn a_ledge_can_be_dropped_off_but_not_climbed() {
    ...
    let above = g.at(Sq::new(0, 0));
    let below = g.at(Sq::new(0, 2));
...
fn a_path_cannot_be_reversed() {
    ...
    let mut reversed = there.steps().to_vec();
    reversed.reverse();
    assert_eq!(
        reversed,
        back.steps(),
...
fn a_direction_blind_cost_still_works_and_is_symmetric() {
    ...
    let a = g.at(Sq::new(0, 2));
    let b = g.at(Sq::new(4, 2));
