...
fn path_toward_breaks_a_perfect_tie_the_same_way_every_time() {
    ...
    let me = g.index_of(Sq::new(4, 4)).unwrap();
    let enemy = g.index_of(Sq::new(8, 4)).unwrap();
    let m = open(&g);
...
fn path_toward_is_stable_across_freshly_built_grids() {
    ...
    let expect = {
        ...
        let m = open(&g);
        let me = g.index_of(Sq::new(5, 5)).unwrap();
        let enemy = g.index_of(Sq::new(10, 10)).unwrap();
        g.path_toward(me, enemy, 30, &m).unwrap().steps
    };
    ...
    for _ in 0..20 {
        ...
        let me = g.index_of(Sq::new(5, 5)).unwrap();
        let enemy = g.index_of(Sq::new(10, 10)).unwrap();
        assert_eq!(g.path_toward(me, enemy, 30, &m).unwrap().steps, expect);
    }
}
...
fn reach_comes_back_in_the_same_order_every_time() {
    let g = FullGrid::square(9, 9, Adjacency::Eight);
    let m = open(&g);
    let start = g.index_of(Sq::new(4, 4)).unwrap();
...
fn path_is_stable_when_several_routes_cost_the_same() {
    ...
    let b = g.index_of(Sq::new(11, 11)).unwrap();
...
fn indices_follow_the_order_the_cells_were_given_in() {
    ...
    for _ in 0..10 {
        ...
        assert_eq!(g.coord(0), Sq::new(0, 0));
        assert_eq!(g.coord(1), Sq::new(1, 0));
        assert_eq!(g.coord(4), Sq::new(0, 1));
